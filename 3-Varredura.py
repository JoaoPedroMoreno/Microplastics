    ...
    with open(caminho_arquivo, 'r') as file:
        for linha in file:
            ...
            if linha.startswith('%freq[Hz]'):
                ...
                agua = 20 * np.log10(abs(valores[3] + 1j * valores[4]))
                gL_0_1 = 20 * np.log10(abs(valores[1] + 1j * valores[2]))
...
def plotar_varredura(df):
    ...
    frequencia_correspondente = frequencia_correspondente*1e9
...
df = gerar_df('Cap_Copo_150um(2).dat')
